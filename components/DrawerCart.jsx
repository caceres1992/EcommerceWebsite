        ...
        <div className={`${drawerIsOpen ? 'w-96' : 'w-0'} transition-all ease-in-out overflow-hidden  bg-gray-50  h-full flex flex-col justify-between  py-3  top-0 fixed right-0`}>
            <div className='px-3 pb-3 flex justify-between border-b-2 border-gray-300'>
                ...
                <p className='text-sm font-medium'>
                    {totalAllProducts} Products - Sub total:
                    <span className='text-xl text-blue-700 tracking-[1px]'>
                        ${totalPrice > 0 ? (totalPrice).toFixed(2) : totalPrice}</span>
                </p>
            ...
            <div className=' pt-3 px-3 border-gray-300  border-t-2'>
                <Link href={"/mycart"} onClick={() => {
                    setDrawerIsOpen(false)
                }} className='py-2 block text-center w-full font-semibold tracking-[1px] bg-black text-gray-50 rounded-sm '>Cheking </Link>
            </div>
        </div>
    )
}
